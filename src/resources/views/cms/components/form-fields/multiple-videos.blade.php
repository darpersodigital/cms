...
<div class="form-input-container multiple-images-container">
                ...
                @foreach ($value as $file)
                    @if (getType($file) == 'string')
                        <div class="col-auto single-multiple-image mb-3" data-image="{!! $file !!}">
                            ...
                            <div class="bg-danger text-white delete-btn" data-image="{{ $file }}" data-testid="remove-current-video-{{$loop->index }}-{{$locale ? $locale .'-' :""}}{{$testID ?? ""}}">
    ...
    @endif
    <label class="custom-file-wrapper placeholder" data-text="Upload Videos">
        <input type="file" class="custom-form-input" multiple name="{{ $input_name }}[]" value="" data-testid="videos-input-input-{{$locale ? $locale .'-' :""}}{{$testID ?? ""}}">
    </label>
