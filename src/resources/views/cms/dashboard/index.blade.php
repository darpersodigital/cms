                ...
                <div class="col-lg-4 mt-3 h-100">
                    <a class="white-card bg-primary w-100 d-flex align-items-center justify-content-center flex-column"
                        ...
                        <h6 class="text-center text-white">
                            ...
                            @else
                                {{ $postType->display_name }}
                            @endif

                        </h6>
                        @if ($postType->is_form)
                            @php $model = 'App\\Models\\' . $postType->model_name;
                                $posts = $model::where('read', '!=', 1)->get();
                            @endphp
                            <div class="bg-warning d-flex align-items-center justify-content-center" style="color:#000;border-radius:50%;height:30px;width:30px;position: absolute;top: -15px;right: -15px;"><b>{{ count($posts) }}</b></div>
                        @endif
                    </a>
                </div>
            @endif
...
@section('scripts')
    {{-- @if (isset($settings->google_analytics_client_id) && !empty($settings->google_analytics_client_id))
        <script>
        ...
        <script>
            ...
            gapi.analytics.ready(function() {
                ...
                gapi.analytics.auth.authorize({
                    ...
                    clientid: '{{ $settings->google_analytics_client_id }}'
                });
                ...
                var viewSelector = new gapi.analytics.ext.ViewSelector2({
                        ...
                        propertyId: '{{ $settings->google_analytics_client_id }}'
                    })
                    .execute();
