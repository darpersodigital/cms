...
@php
    $base_url = config('cms_config.route_path_prefix') . '/sitemaps';
@endphp
...
@section('dashboard-content')
    <div class="container-fluid px-md-5  mt-3">
        ...
        @include('darpersocms::cms.components.breadcrumb.ScreenTitleHeader',[
            'title'=>"Sitemap",
            'can_add'=> request()->get('admin')['post_types']['sitemaps']['permissions']['add'],
            'can_delete'=> request()->get('admin')['post_types']['sitemaps']['permissions']['delete'],
            'testID'=> 'sitemaps'
        ])
                        ...
                        @foreach ($rows as $row)
                            <tr>
                                ...
                                @include('darpersocms::cms.post-type._includes.row-actions',[
                                    'can_edit'=>request()->get('admin')['post_types']['sitemaps']['permissions']['edit'],
                                    'can_delete'=>request()->get('admin')['post_types']['sitemaps']['permissions']['delete'],
                                    'testID'=>'sitemaps'
                                ])
