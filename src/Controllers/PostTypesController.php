...
use Darpersodigital\Cms\Controllers\CustomCRUDController;
...
class PostTypesController extends BaseController
{
    ...
    protected CustomCRUDController $customCRUD_controller;

    public function __construct(DatabaseController $db_controller, SitemapServices $sitemap_services,CustomCRUDController $customCRUD_controller )
    {
        $config = include __DIR__ . '/constants/constants.php';
        $this->db_controller = $db_controller;
        $this->customCRUD_controller = $customCRUD_controller;
    ...
    {
        $rows = PostType::orderBy('pos', 'asc')->where('hidden', 0)->get();
    ...
    public function edit($id)
    {
        ...
        $migration_types = $this->migration_types;
        $form_fields = $this->form_fields;
        return view('darpersocms::cms/post-types/create', compact('post_type', 'migration_types', 'form_fields'));
    }

    public function editCustom($id)
    {
        $post_type = PostType::where('custom_page', 1)->findOrFail($id);
        return view('darpersocms::cms/post-types/create-custom', compact('post_type'));
    }
    ...
    private function storeOrUpdateCustom(Request $request, $id)
    {
        ...
        } elseif (!$request->display_name_plural) {
    ...
    private function validatePostType(Request $request, $id = null)
    {
        ...
        if (!$id) {
            $rules = array_merge($rules, [
                ...
                'translatable_name.*' => 'required|distinct',
        ...
        $validator->after(function ($validator) use ($request) {
            ...
            $translatableNames = $request->input('translatable_name', []);

            $duplicates = array_intersect($names, $translatableNames);

            if (!empty($duplicates)) {
                $validator->errors()->add('name', 'name and translatable_name must not contain the same values: ' . implode(', ', $duplicates));
            }
        });
        $validator->validate();
    }
    ...
    private function storeOrUpdate(Request $rq, $id)
    {
        ...
        if (isset($rq->with_seo) && $rq->with_seo) {
            $seoFields = ['seo_image', 'seo_title','seo_page_title', 'seo_description', 'seo_keywords','seo_robots','seo_og_type'];
            $seoMigrationTypes = ['text', 'text','text','text', 'text', 'text','text'];
            $seoFormFields = ['image with alt', 'text','text','text', 'text', 'text','text'];
            $seoAdditionalValidations = ['', 'max:512', 'max:191', 'max:1600', 'max:1024','',''];
            $defaultNulls = array_fill(0, 7, null);
            $defaultZeros = array_fill(0, 7, '0');
            $defaultOnes = array_fill(0, 7, '1');
            $emptyStrings = array_fill(0, 7, '');
            $fieldsToMerge = [
        ...
        if (isset($request->is_form)) {
            $existingNames = $request->input('name', []);
            // Only proceed if neither 'read' nor 'star' exist

            if (!in_array('read', $existingNames) && !in_array('star', $existingNames)) {
                $is_form_fields = [
                    [
                        'name' => 'read',
                ...
                $fields = array_merge($fields, $is_form_fields);
                $fieldsToAppend = ['name', 'migration_type', 'form_field', 'form_field_configs_1', 'additional_validations', 'form_field_configs_2', 'description', 'hide_table', 'can_create', 'can_read', 'can_update', 'nullable', 'unique'];
                $merged = [];
                foreach ($fieldsToAppend as $key) {
                    $existing = $request->input($key, []);
                    $append = array_map(fn($field) => $field[$key] ?? null, $is_form_fields);
                    $merged[$key] = array_merge($existing, $append);
                }
        ...
        if (!is_array($translatable_fields)) {
            return $translatable_fields;
        }

        if (count($request['old_name']) != count($request['name']) && $id) {
            abort(500);
        }
        ...
        if (!$id) {
            ...
            if (!is_null($edit_db_res)) {
                return $edit_db_res;
            }
            if ($post_type->database_table != $request->database_table) {
        ...
        $post_type->custom_crud = isset($request->custom_crud) ? 1 : 0;
        $post_type->has_sitemap = isset($request->has_sitemap) ? 1 : 0;

        if (isset($request->custom_crud)) {
            $this->customCRUD_controller->createCustomCRUDController($post_type);
        }
    ...
    public function destroy($id)
    {
        ...
        if (!count($array)) {
            return redirect(config('cms_config.route_path_prefix') . '/post-types')->with('error', 'No record selected');
        }
        foreach ($array as $id) {
            $this->deletePage($id);
        }
            ...
            for ($i = 0; $i < count($name); $i++) {
                ...
                foreach ($fields as $field) {
                    if ($field['name'] == $name[$i]) {
                        throw ValidationException::withMessages(['Column "' . $name[$i] . '" already exists']);
                    }
                }
                ...
                if ($form_field[$i] != 'select multiple') {
                    if (!$migration_type[$i]) {
                        throw ValidationException::withMessages(['The ' . ($isTranslation ? 'translatable_' : '') . 'migration_type.' . $i . ' field is required.']);
                    } elseif (!in_array($migration_type[$i], $this->migration_types)) {
                        throw ValidationException::withMessages(['The ' . ($isTranslation ? 'translatable_' : '') . 'migration_type.' . $i . ' field is not valid.']);
                    }
                }
                ...
                $fields[] = [
                    ...
                    'form_field_configs_1' => $isTranslation ? $request['translatable_form_field_configs_1'][$i] : $request['form_field_configs_1'][$i],
                    'form_field_configs_2' => $isTranslation ? $request['translatable_form_field_configs_2'][$i] : $request['form_field_configs_2'][$i],
                ];
            }
        }
