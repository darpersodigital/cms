...
class DatabaseController extends BaseController
{
        ...
        if ($post_type->database_table) {
            foreach (json_decode($post_type['fields'], true) as $field) {
                if ($field['form_field'] == 'select multiple') {
                    $pivot_table =  $field['name'].'_'. Str::singular($post_type->database_table) ;
    ...
    private function managePivotTables($request, $old_page = null)
    {
        foreach ($request->form_field as $f => $form_field) {
            if ($form_field == 'select multiple') {
                ...
                $column_name = $request->form_field_configs_1[$f] == $request->database_table ? 'other_' . Str::singular($request->form_field_configs_1[$f]) . '_id' : Str::singular($request->form_field_configs_1[$f]) . '_id';
    ...
    private function createPivotSchema($pivot_table, $column_name, $request, $index, $includeExtraFields)
    {
        Schema::create($pivot_table, function ($table) use ($column_name, $request, $index, $includeExtraFields) {
            ...
            $table
                ->foreign(Str::singular($request->database_table) . '_id')
                ->references('id')
                ->on($request->database_table)
                ->onDelete('cascade');
        });
    }
    ...
    private function modifyMigrationTypesAndNullable($request, $old_page, $isTranslation)
    {
        ...
        foreach ($request[$nameKey] as $i => $name) {
            ...
            if (in_array($formField, $excludedField, true)) {
                continue;
            }
        ...
        foreach ($rqName as $i => $name) {
            if (in_array($field_name[$i], ['select multiple'], true)) {
                continue;
            }

            $tableName = $request['database_table'] . $tableSuffix;

            $columnType = $isTranslation ? $request->translatable_migration_type[$i] : $request->migration_type[$i];

            $isNullable = $isTranslation ? !empty($request->translatable_nullable[$i]) : !empty($request->nullable[$i]);

            // If column is becoming NOT NULL, replace existing null values first
            if (!$isNullable) {
                $defaultValue = match ($columnType) {
                    'string', 'text', 'longText', 'mediumText' => '',
                    'integer', 'bigInteger', 'smallInteger', 'tinyInteger', 'mediumInteger' => 0,
    ...
    public $translatedAttributes = ' .
                json_encode($request['translatable_name']) .
                ';';
            $translated_attributes =
                $translated_attributes .
                '
    protected $translationForeignKey ="' .
                Str::singular($request['database_table']) .
                '_id' .
                '";';
        }
...
';
        ...
        $this->createModelFile($request['model_name'], [$head, $request['model_name'], $implements, $request['database_table'], $use, $translated_attributes, $body, $custom_functions]);

        if ($request['translatable_name']) {
            $replacements['model_name'] = $request['model_name'] . 'Translation';
            $replacements['database_table'] = $request['database_table'] . '_translations';

            $this->createModelFile($request['model_name'] . 'Translation', ['', $request['model_name'] . 'Translation', '', $request['database_table'] . '_translations', '', '', '', $custom_functions]);
        }
    }
    ...
    private function createModelFile($modelName, $replacements)
    {
        ...
        $fileContent = str_replace(['%%head%%', '%%model_name%%', '%%implements%%', '%%database_table%%', '%%use%%', '%%translated_attributes%%', '%%body%%', '%%custom_functions%%'], $replacements, file_get_contents(__DIR__ . '/constants/model.stub'));
